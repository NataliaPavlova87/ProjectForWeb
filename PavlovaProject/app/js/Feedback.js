class Feedback {
    ...
    _initForm(){
        $(this.form).submit(e => {
            e.preventDefault();
            let $errorText = $('.fileds');
            $errorText.empty();

            if (!$('#author').val() && !$('#text').val()){
                $errorText.text('* Required Fileds');
                return;
            }
            let comment = {
                id: ++this.curID,
    ...
    _renderComment(comment){
        ...
        let $delBtn = $('<span class="details-right__elements details-right-span"><i class="fas fa-times-circle det-rgh-i"></i></span>');
